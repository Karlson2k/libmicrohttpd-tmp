...
#ifdef USING_SQLITE3
static struct MHD_Response *db_response = 0;
#endif
  ...
  if(0 == strcmp (url, "/db"))
    {
        sqlite3 *db = 0;
        int rc;
        void *uData = MHD_get_connection_daemon_udata (connection);
        db = (sqlite3*)uData;

        const char db_html[] = "<html><head><title>DB test</title></head><body>DB test</body></html>";
        if(!db)
        {
        ...
        sqlite3_stmt *stmt;
        const char *select = "select * from companies where id=%d;";
        //const char *select = "select * from companies where name like '%dad%';";
        int si, sid = 10;
        for(si=0; si < 10; ++si, sid += 1000)
        {
            char buf[256];
            snprintf(buf, sizeof(buf), select, sid);
            rc = sqlite3_prepare(db, buf, strlen(buf),&stmt, 0);
            if(rc == SQLITE_OK)
            {
        ...
        if(!db_response)
        {
            db_response = MHD_create_response_from_buffer (strlen (db_html),
							     (void *) db_html,
							     MHD_RESPMEM_MUST_COPY);
            mark_as_html (db_response);
        }
        return MHD_queue_response (connection,
				   MHD_HTTP_OK,
				   db_response);
...
#ifdef USING_SQLITE3
void *create_daemon_userdata()
{
    sqlite3 *db = 0;
    int rc = sqlite3_open("/home/mingo/dev/companies-uk/companies-uk-RG.db", &db);
    //printf("Opening sqlite3 %p\n", db);
    return db;
}

void destroy_daemon_userdata(void *udata)
{
    sqlite3 *db = (sqlite3*)udata;
    if(db)
    {
        //printf("Closing sqlite3 %p\n", db);
        sqlite3_close(db);
    }
}
#endif
...
#ifdef USING_SQLITE3
                        MHD_OPTION_DAEMON_CREATE_UDATA, &create_daemon_userdata,
                        MHD_OPTION_DAEMON_DESTROY_UDATA, &destroy_daemon_userdata,
#endif
			MHD_OPTION_END);
  if (NULL == d)
    return 1;
